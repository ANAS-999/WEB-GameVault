...
async function fetchMainTimeline(gameName) {
  ...
  if (Array.isArray(data)) {
    const dlcKeywords = [
      'quest', 'contract', 'dlc', 'expansion', 'episode', 'soundtrack', 'pack',
      'bundle', 'collection', 'trilogy', 'remastered', 'edition', 'addon', 'online',
      'pass', 'guide', 'vr', 'ifruit', 'london', 'complete', 'artbook', 'mod',
      'scavenger', 'gear', 'beard', 'alternative look', 'armor', 'hearts of stone', 'blood and wine',
      'ballad of gay tony', 'lost and damned'
    ];

    const seenMainTitles = new Set();
    const filtered = data.filter(g => {
      const lower = g.name.toLowerCase();

      // Exclude DLCs/quests/expansions/modpacks
      if (dlcKeywords.some(kw => lower.includes(kw))) return false;

      // Extract core main title
      const cleanKey = lower
        .replace(/^(the\s+)/i, '')
        .replace(/:\s*(wild hunt|assassins of kings|enhanced|remastered|collector's edition|digital deluxe|premium edition|launch edition|definitive edition|special edition).*/i, '')
        .replace(/[^a-z0-9]/gi, '');

      if (seenMainTitles.has(cleanKey)) return false;
      seenMainTitles.add(cleanKey);
      return true;
    });

    console.log('Main Standalone Story Line Games:');
    filtered.forEach(g => {
...
async function run() {
  await fetchMainTimeline('The Witcher 3: Wild Hunt');
  await fetchMainTimeline('God of War Ragnarök');
  await fetchMainTimeline('Grand Theft Auto V');
}
